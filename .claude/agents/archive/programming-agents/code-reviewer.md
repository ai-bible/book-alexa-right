...
1. **Run Static Analysis First**: BEFORE starting manual review, run static analysis tools:
   - **Ruff**: `make rlint` for fast linting (unused imports, undefined names, etc.) + verify code formatting
   - **MyPy**: `make mypy` for type checking (if configured in project)
...
1. Run `make rlint` for linting issues + formatting issues
2. Run `make mypy` for type checking (if configured)
