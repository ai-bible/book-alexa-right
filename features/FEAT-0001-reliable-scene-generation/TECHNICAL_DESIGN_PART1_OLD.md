...
#### Purpose
Validate that scene blueprint is complete, internally consistent, and compliant with v3 plan changes BEFORE expensive generation begins. Fail-fast principle in action.
...
- **scene_id** (string): Scene identifier
  - Example: "0204"
- **plan_v3_constraints** (optional, string): Path to plan-v3 excerpt with documented changes
  - Example: `acts/act-1/plan-v3-excerpts/chapter-02-constraints.md`
...
// File: workspace/validation/scene-{ID}-constraints-list.json
{
  ...
  "validation_checks": [
    {"check": "version_header_present", "result": "PASS"},
    {"check": "v3_changes_documented", "result": "PASS"},
  ...
  "errors": [
    {
      ...
      "severity": "HIGH",
      "message": "Blueprint lacks version header. Cannot determine if v3 compliant.",
      "fix": "Add version header following Rule 6 format"
    },
    {
      "type": "contradiction",
      "severity": "HIGH",
...
2. **Extract Metadata**
   - Parse version from header (e.g., "Version: v3.0")
   - IF no version → WARNING (can continue but flag)
   - Parse status (e.g., "Status: APPROVED FOR GENERATION")
   - Parse date

3. **Check Version Compliance**
   - IF version < v3.0 → ERROR: "Blueprint is v{X}, requires v3.0+"
   - IF "V3 CRITICAL CHANGES" section exists → Parse removed/changed/added items

4. **Extract Critical Requirements**
   ...
   - **Characters**: Extract from "Characters:" section
     ...
     - Check for v3 removed characters (Себастьян Грей)
...
6. **Conflict Checks** (if plan_v3_constraints provided)
   - Compare blueprint against documented plan-v3 changes
   - Flag any elements marked "REMOVED in v3" that appear in blueprint
   - Flag any "CHANGED in v3" elements that use old form
...
- **Rule 6 (Version Tagging)**: Enforces version header requirement
...
- **Multiple Versions**: If multiple version files exist (e.g., scene-0204-blueprint-v2.md, scene-0204-blueprint.md), use ONLY the exact path provided - never guess
- **v3 Changes**: Maintains internal list of known v3 changes (Себастьян removal, location standardization, mechanics changes) for cross-validation
...
**Explicitly ABSENT**:
- Себастьян Грей (removed in plan v3 - will NOT appear)
...
- [ ] Себастьян Грей does NOT appear (v3 removal)
...
3. **Format Characters Section**
   ...
   - List absent characters with reasons (especially v3 removals)
...
4. **Format Plot Beats**
   ...
   - Highlight any splot consequences or emotional arcs
...
- MUST NOT BE PRESENT: Себастьян Грей (removed in v3)
...
**PRIMARY BLUEPRINT**: [exact file path]
**VERIFIED PLAN**: [exact file path to verified-plan.json]
Version: v3.0 FINAL
Status: APPROVED FOR GENERATION

⚠️ DO NOT USE:
- Any other blueprint versions
- Draft or revised files
- Workspace temporary files

IF FILES DO NOT EXIST: STOP and return error
...
**Beat 2**: Погружение - память о смерти дочери
**REMINDER**: Себастьян Грей does NOT appear (v3 removal)
...
- **Rule 5 (Single Source of Truth)**: Exact file paths specified
...
2. **Character Presence Check** (<5 seconds)
   ...
   - Required absent: Extract from constraints (especially v3 removals)
...
4. **Integration with Existing Workflows**
   ...
   - Migration plan (adding version headers to old blueprints)
